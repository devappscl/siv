...
function ComprobarStock($id){
    ...
    $query = $db->query("SELECT cantidad FROM stock WHERE producto ='$id'");
...
function ComprobarSalida($id){

    $db = \Config\Database::connect();

    $query = $db->query("SELECT SUM(cantidad) AS total FROM ventadetalle WHERE created_at >= DATE(NOW() - INTERVAL 7 DAY) AND producto_id ='$id'");
...
function ComprobarStockSucursal($id,$sucursal){

    $db = \Config\Database::connect();

    $query = $db->query("SELECT cantidad FROM stock WHERE producto ='$id' AND sucursal = '$sucursal'");
